...
<Description>モーダルダイアログ。フォーカストラップ・Escape キー・オーバーレイクリックでの閉じ操作に対応。</Description>

## Installation

<CodeBlock code={`npx shadcn@latest add https://kasumi-ui.vercel.app/r/dialog`} />
...
<ComponentPreview>
  <Dialog>
    ...
    <DialogContent>
      ...
      <DialogDescription>{"表示名とメールアドレスを変更できます。"}</DialogDescription>
...
<ComponentPreview>
  <Dialog>
    ...
    <DialogContent>
      ...
      <DialogDescription>{"この操作は取り消せません。すべてのデータが完全に削除されます。"}</DialogDescription>
      <DialogFooter>
        <DialogClose asChild>
          <Button variant="outline">キャンセル</Button>
        </DialogClose>
